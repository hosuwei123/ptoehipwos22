...
def get_table(stock_id):
    ...
    time.sleep(random.randrange(6,13 ))  #延遲6-13秒防鎖ip
...
def seasoneps(ur20,headers):#每季收入
    # res=requests.get(ur20,headers=headers)
    # res.encoding='utf-8'  
    #-------下拉式選單
    driver = webdriver.Chrome()#建立webdriver物件，傳入剛剛所下載的「瀏覽器驅動程式路徑」及「瀏覽器設定(chrome_options)」，其中的「瀏覽器驅動程式路徑」一定要傳入，而「瀏覽器設定(chrome_options)」則可視情況傳入，為選擇性的。
    ...
    select.select_by_value("IS_M_QUAR")#方法 2：透過 option 的 value 值選取資料, 執行後會選取「選項 2」選項
